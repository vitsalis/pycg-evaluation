...
This artifact contains the following materials:

1. The source code of our prototype implementation `PyCG` (provided as a Git
   submodule).
2. The source code of the tools we compare `PyCG` against, namely `Pyan` and
...
software under the Apache 2.0 Licence, and can also be reached through the
...
`pycg-eval`).
Run the following command to create and enter a new container.
```bash
docker run -ti --rm pycg-eval
```

After executing the command you will enter the home directory (i.e.,
...
Having `PyCG` installed we can use it through the `pycg` command.
...
If not already inside the Docker environment, we enter it by executing
```bash
docker run -ti --rm pycg-eval
```

Then, we navigate to the `/example` directory and use `PyCG` to generate the
call graph.
...
format the keys of the JSON output correspond to the nodes of the graph and are
each assigned a list. Each entry in the list corresponds to an edge stemming from
...
Let's exit the container so that we can proceed to the evaluation.
...
We will start by evaluating the micro-benchmark described in Section 5.1 of the
...
`Depends`. To do so, we create and enter the `pycg-eval` container.
...
container. This allows us to access the `data/micro-benchmark`, `scripts` and
...
Now, we will evaluate the macro-benchmark described in Section 5.1 of the
paper. An overview of its contents can be viewed on Table 2.
...
To do that, let's create and enter the Docker container.
...
First, let's generate the `PyCG` call graphs. Note that the generated call
...
We will do the same for `Pyan`, passing a call graph convertion script as an
...
Finally, we generate call graphs using `Depends`. The generated call graphs
...
Having generated the call graphs for the macro-benchmark projects, we can
...
readable format. To do so, we employ the `table4.py` script
...
human readable format. To do so, we invoke
